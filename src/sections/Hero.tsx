...
export function Hero() {
  ...
  const cursorGlow = useMotionTemplate`radial-gradient(38vmin circle at ${sxPct} ${syPct}, rgba(111,227,255,0.09) 0%, rgba(111,227,255,0.02) 45%, transparent 68%)`

  const vaybGlow = useSpring(0, { stiffness: 60, damping: 28 })

  const d = (ms: number) => (reduceMotion ? 0.01 : ms)

  const b1Initial = reduceMotion
    ? { opacity: 1, y: 0 }
    : { opacity: 0, y: 20 }
  ...
  const b1Transition = {
    ...
    delay: reduceMotion ? 0 : HERO_DELAY_B1,
  }

  const b2Initial = reduceMotion
  ...
  const b2Transition = {
    ...
    delay: reduceMotion ? 0 : HERO_DELAY_B2,
  }

  const b3Initial = reduceMotion
    ? { opacity: 1, y: 0, filter: 'blur(0px)' }
    : { opacity: 0, y: 14, filter: 'blur(11px)' }
  const b3Animate = { opacity: 1, y: 0, filter: 'blur(0px)' }
  const b3Transition = {
    duration: d(HERO_DUR_B3),
    ease: HERO_EASE_BLOCK3,
    delay: reduceMotion ? 0 : HERO_DELAY_B3,
  }

  const accentGlowInitial = reduceMotion ? { opacity: 1 } : { opacity: 0.38 }
  const accentGlowAnimate = { opacity: 1 }
  const accentGlowTransition = {
    duration: d(HERO_ACCENT_GLOW_DUR),
    ease: HERO_EASE_BLOCK3,
    delay: reduceMotion ? 0 : HERO_ACCENT_GLOW_DELAY,
  }

  const manifestRevealEnd = reduceMotion ? 0.08 : HERO_DELAY_B3 + HERO_DUR_B3
  ...
  const imageExtraGlow = useTransform(vaybGlow, [0, 1], [0.22, 0.55])
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(105deg, rgba(15,17,19,0.92) 0%, rgba(15,17,19,0.55) 38%, rgba(15,17,19,0.18) 58%, rgba(15,17,19,0.06) 72%, transparent 100%)',
        }}
      />
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div
          ...
          style={{
            background:
              'radial-gradient(circle, rgba(201,204,209,0.1) 0%, rgba(42,38,35,0.5) 45%, transparent 70%)',
          }}
        />
        <div
          ...
          style={{
            background:
              'radial-gradient(circle, rgba(58,55,51,0.45) 0%, rgba(15,17,19,0.25) 55%, transparent 72%)',
            animationDelay: '-7s',
          }}
        />
        <div
          className="hero-ambient-orb--soft absolute left-[35%] top-[42%] h-[40vmin] w-[48vmin] rounded-full blur-[110px] md:left-[40%] md:top-[38%]"
          style={{
            background:
              'radial-gradient(ellipse 100% 100% at 50% 50%, rgba(111,227,255,0.06) 0%, rgba(42,38,35,0.35) 50%, transparent 72%)',
            animationDelay: '-14s',
          }}
        />
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-y-14 md:gap-y-16 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:gap-x-8 lg:gap-y-0 xl:gap-x-12">
          <div className="order-1 flex min-w-0 flex-col justify-center lg:order-1 lg:max-w-none lg:pr-2 xl:pr-6">
              ...
              >
                ...
                >
                  <span className="hero-manifest__stanza hero-manifest__stanza--bridge block">
                    <HeroBreathPhrase delay={BREATH_DELAYS[0]} reduceMotion={!!reduceMotion}>
                      <span>
                        которые{' '}
                ...
                >
                  <span className="hero-manifest__stanza hero-manifest__stanza--bridge block">
                    <HeroBreathPhrase delay={BREATH_DELAYS[1]} reduceMotion={!!reduceMotion}>
                      <motion.span
                        ...
                        onPointerEnter={() => vaybGlow.set(1)}
                        onPointerLeave={() => vaybGlow.set(0)}
                      >
                        <span
                          className="hero-manifest-accent__text hero-manifest-accent__text--post-shimmer"
                          style={{
                            animationDelay: reduceMotion ? '0s' : `${HERO_POST_SHIMMER_VIBE_S}s`,
                          }}
                        >
                          как вайб
                        </span>
                ...
                <span className="hero-manifest__finale">
                  ...
                  >
                    <span className="hero-manifest__punch">
                      <HeroBreathPhrase delay={BREATH_DELAYS[2]} reduceMotion={!!reduceMotion}>
                        <span>
                          как{' '}
                          <motion.span
                            className="hero-manifest-accent hero-manifest-accent--inline"
                            ...
                            transition={{
                              ...
                              delay: reduceMotion ? 0 : HERO_ACCENT_GLOW_DELAY + 0.06,
                            }}
                          >
                            <span
                              className="hero-manifest-accent__text hero-manifest-accent__text--post-shimmer"
                              style={{
                                animationDelay: reduceMotion ? '0s' : `${HERO_POST_SHIMMER_SYSTEM_S}s`,
                              }}
                            >
                              система
                            </span>
            ...
            <motion.p
              ...
              transition={{
                ...
                delay: reduceMotion ? 0.06 : Math.max(0, manifestRevealEnd - 0.12),
              }}
            ...
            <motion.div
              ...
              transition={{
                ...
                ease: [0.22, 1, 0.36, 1] as const,
                delay: reduceMotion ? 0.1 : manifestRevealEnd + 0.06,
              }}
          ...
          >
            <div className="relative w-[min(112%,42rem)] max-w-[none] lg:w-[min(118%,46rem)]">
              ...
              >
                <motion.div
                  className="pointer-events-none absolute -inset-[8%] z-0 rounded-[2px] blur-3xl"
                  style={{
                    opacity: imageExtraGlow,
                    background:
                      'radial-gradient(ellipse 70% 60% at 50% 45%, rgba(111,227,255,0.18) 0%, transparent 65%)',
                  }}
                />
                <div
                  className="relative z-[1] overflow-visible"
                  style={{
                    filter: reduceMotion ? undefined : 'drop-shadow(0 24px 48px rgba(0,0,0,0.35))',
                  }}
                >
                  <div
                    className="relative"
                  ...
                  >
                    <div
                      ...
                      style={{
                        background:
                          'linear-gradient(90deg, rgba(15,17,19,0.5) 0%, transparent 45%, rgba(111,227,255,0.04) 100%)',
                    ...
                    <div
                      ...
                      style={{
                        background:
                          'linear-gradient(180deg, transparent 55%, rgba(15,17,19,0.35) 100%)',
                      }}
                    />
                    <img
                      ...
                      width={1200}
                      height={900}
                      className="relative z-[1] h-auto w-full scale-[1.12] object-cover object-[center_42%]"
                      decoding="async"
                      fetchPriority="high"
                    />
                  </div>
